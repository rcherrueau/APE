...
;; Core lang that defines `module-begin` with the workflow followed by
;; all extension of the language. The workflow consists in:
;; 1. Call macro expansion on the program to build the AST.
;; 2. Call `compile-exp` of calling site on that AST to build ASM.
;; 3. Get and print the textual form of ASM.
