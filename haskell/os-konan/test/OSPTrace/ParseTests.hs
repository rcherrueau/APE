module OSPTrace.ParseTests (testsAll) where
...
import Data.OpenStack.OSPTrace
...
assertParsable file = do
  ...
  let res  = eitherDecodeTrace json :: Either String OSPTrace
...
testsDBReq = TestLabel "DB Request Parsing" $ TestList
  ...
  -- ]
  [ TestCase $ assertOSP dbreq (DBReq select_1)
  , TestCase $ assertOSP dbreqparams (DBReq select_1)
  ]

  where
    select_1 :: Maybe QueryExpr
    select_1 = Just Select {
...
testsTraces = TestLabel "Trace Parsing" $ TestList $
  ...
  [ "rsc/osprofiler/empty-root.json"
  , "rsc/osprofiler/wsgi.json"
  , "rsc/osprofiler/server-create.json"
  ]
