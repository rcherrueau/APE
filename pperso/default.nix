...
# let cKanren = racket2nix.buildRacketPackage "git://github.com/rcherrueau/rastache.git#master";
# in  
pkgs.mkShell {
  # buildInputs = [ pkgs.racket cKanren ];
  buildInputs = [ pkgs.racket ];
  shellHook = ''
    echo 'Run: raco pkg install "git://github.com/rcherrueau/rastache.git?path=rastache#master"'
  '';
}
